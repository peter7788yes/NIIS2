...
public partial class System_PowerM_RolePowerSetting_Add : BasePage
{
    ...
    protected new void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.HttpMethod.Equals("POST"))
        {
            ...
            {
                int SystemPowerCateID = Convert.ToInt32(WebConfigurationManager.AppSettings["SystemPowerCateID"]);
                var list = SystemPowerCate.list;
                if (SystemPowerCateID == 1)
                {
                    foreach (var item in SystemPowerCate.list)
                    {
                        ddlCate.Items.Add(new ListItem() { Text = item.SystemPowerCateName, Value = item.ID.ToString() });
                    }
                }
                else
                {
                    var query = SystemPowerCate.list.Where(item => item.ID == SystemPowerCateID).First();
                    ddlCate.Items.Add(new ListItem() { Text = query.SystemPowerCateName, Value = query.ID.ToString() });
            ...
            ddlCate.SelectedValue = tmpRoleCateID == 0 ? "" : tmpRoleCateID.ToString();
