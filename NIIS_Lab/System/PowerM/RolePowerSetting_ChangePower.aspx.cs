...
public partial class System_PowerM_RolePowerSetting_ChangePower : BasePage
{
    ...
    protected new void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.HttpMethod.Equals("POST"))
        {
                ...
                Dictionary<string, object> OutDict = new Dictionary<string, object>() { { "@RoleCateID", ModuleCateID } };
    ...
    private string GetMenu()
    {
        ...
        DataTable dt = MSDB.GetDataTable("ConnUser", "dbo.usp_SystemM_xGetModuleMenuByModuleCateID"
                                          , new Dictionary<string, object>()
                                          {
                                                    { "@UserID", 1 },
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (RoleID == 0)
        {
            string SC = "<style>body{display:none;}</style><script>alert('資料取得失敗');location.href = '/System/PowerM/RolePowerSetting.aspx';</script>";
