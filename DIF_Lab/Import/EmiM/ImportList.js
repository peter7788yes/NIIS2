...
angular.module("MyApp", ["PageM", "TableM", "FilterM"])
         .controller("MyController", ["$scope", "PageProvider", "TableProvider", function ($scope, PageProvider, TableProvider) {
             ...
             $scope.goDetail = function (record) {
                 location.href = "ImportLogDetail.aspx?id=" + record["ID"] + "&s=" + record["SuccessCnt"] + "&r=" + record["RepeatCnt"] + "&e=" + record["ErrorCnt"] + "&u=" + record["UserName"];
             }

             $scope.getError = function (record) {
                 location.href = "ErrorList.aspx?id=" + record["ID"];
             }
             //西元年轉換民國年
             ...
             $scope.Search = function (pageIndex) {
                 ...
                 if ($("#UserName").val().trim() != "") {
                     for (var i = 0; i < data.length; i++) {
                         var tmpUser = data[i].split("#");
                         if ($("#UserName").val().trim() == tmpUser[0]) {
                             $scope.VM.UserID = tmpUser[1];
                             break;
                         }
                     }
                     if ($scope.VM.UserID == "") {
                 ...
                 postData.UserID = $scope.VM.UserID;
