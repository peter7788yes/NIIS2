...
public partial class Vaccination_RecordM_StudentRecord_Detail : BasePage
{
    ...
    public Vaccination_RecordM_StudentRecord_Detail()
    {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (this.IsPostBack==false)
        {
            ...
            tbStudent.Text = VM.StudentNumber.ToString();
            StudentNumber = VM.StudentNumber;

            tbCard.Text = VM.HasYellowCardNumber.ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Chk > 0)
        {
            script = "<script>alert('儲存成功');location.href = '/Vaccination/RecordM/StudentRecord.aspx';</script><style>body{display:none;}</style>";
        }
        else
        {
