...
public partial class Vaccine_StockManagementM_VaccineOut_New_VaccineOutBatch : BasePage
{
    ...
    public Vaccine_StockManagementM_VaccineOut_New_VaccineOutBatch()
    {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (this.IsPostBack == false)
        {
            ...
            //if (dt1.Rows.Count > 0)
            //{
            //    OriFroIdx.SelectedValue = dt1.Rows[0]["MonIdx"].ToString();
            //}
        }
    }
    protected void Save_Click(object sender, EventArgs e)
    {
        ...
        if (UploadFileSuccess == true)
        {
            ...
            using (SqlConnection sc = new SqlConnection(WebConfigurationManager.ConnectionStrings["ConnDB"].ToString()))
            {
                using (SqlCommand cmd = new SqlCommand("usp_VaccineOut_xAddVaccineOutBatchData", sc))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@BatchDataID", BatchDataID);
                    cmd.Parameters.AddWithValue("@VaccineOutID", VaccineInID);
